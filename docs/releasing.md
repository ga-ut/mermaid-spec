...
The package is a Bun CLI distributed through the public npm registry.
...
3. Check `npm view mermaid-spec@<version> version dist.integrity` and compare
...
4. From a directory without a local install, use a fresh Bun cache and run
   `bunx mermaid-spec@<version> --version`, then the README's first-specification
